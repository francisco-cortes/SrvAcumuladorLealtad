...
public class CifradorAesUtil {
    ...
    public String encryptAes(String aesKeyBase64, String hmacKeyBase64, String valorCampo) {
        try {
            SecretKey aesKey = new SecretKeySpec(java.util.Base64.getDecoder().decode(aesKeyBase64.getBytes(ParametrerConfiguration.ENCODING_UTF8)),
                    ParametrerConfiguration.AES_KEY);
            SecretKey hmacKey = new SecretKeySpec(java.util.Base64.getDecoder().decode(hmacKeyBase64.getBytes(ParametrerConfiguration.ENCODING_UTF8)),
    ...
    public String decryptAes(String aesKeyBase64, String hmacKeyBase64, String valorCifrado) {
        try {
            SecretKey aesKey = new SecretKeySpec(Base64.getDecoder().decode(aesKeyBase64.getBytes(ParametrerConfiguration.ENCODING_UTF8)),
                    ParametrerConfiguration.AES_KEY);
            SecretKey hmacKey = new SecretKeySpec(Base64.getDecoder().decode(hmacKeyBase64.getBytes(ParametrerConfiguration.ENCODING_UTF8)),
                    ParametrerConfiguration.ALGORITHM_HMAC);
